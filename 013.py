...
def parse_machine(machine_data):
    a_parts = machine_data[0].split(', ')
    a_x = int(a_parts[0].split('+')[1])
    a_y = int(a_parts[1].split('+')[1])
    
    b_parts = machine_data[1].split(', ')
    b_x = int(b_parts[0].split('+')[1])
    b_y = int(b_parts[1].split('+')[1])
    
    prize_parts = machine_data[2].split(', ')
    target_x = int(prize_parts[0].split('=')[1]) + 10000000000000
    target_y = int(prize_parts[1].split('=')[1]) + 10000000000000
    
    return (a_x, a_y), (b_x, b_y), (target_x, target_y)
...
def can_win_prize(a_moves, b_moves, target_x, target_y):
    det = a_moves[0] * b_moves[1] - a_moves[1] * b_moves[0]
    if det == 0:
        return False, 0, 0
        
    det_n = target_x * b_moves[1] - target_y * b_moves[0]
    det_m = a_moves[0] * target_y - a_moves[1] * target_x
    
    if det_n % det != 0 or det_m % det != 0:
        return False, 0, 0
...
def solve(text):
    ...
    for machine in machines:
        a_moves, b_moves, (target_x, target_y) = parse_machine(machine)
        winnable, a_count, b_count = can_win_prize(a_moves, b_moves, target_x, target_y)
