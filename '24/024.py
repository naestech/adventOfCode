...
def parse_input(input_text):
    ...
    for line in sections[1].split('\n'):
        ...
        if len(parts) == 3:  # Binary gate (AND, OR, XOR)
    ...
    while True:
        ...
        for op, in1, in2, out in gates:
            ...
            if in1 in wires and in2 in wires:
                wires[out] = evaluate_gate(op, wires[in1], wires[in2])
                progress = True
        
        if not progress:
            break
    
    # Collect all z-wires in order (z00 should be least significant bit)
    z_wires = sorted([wire for wire in wires if wire.startswith('z')], reverse=True)
    
    # Convert to binary number (reading right to left)
    result = 0
    for wire in reversed(z_wires):  # Process z00 first (least significant bit)
...
def solve(input_text):
    ...
    return simulate_circuit(wires, gates)

print(solve(input_text.strip()))
